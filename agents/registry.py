...
# the issue on the test host with a screenshot posted to the GH issue, and names the REAL surface for grounding.
REPRODUCE_OUT = _obj({
    "summary": _STR,
...
SPEC_AUTHOR_OUT = _obj({
    ...
    # test `type` is GENERIC — unit | integration | e2e | agentic (or a free string). The CONCRETE
    # tool (pytest, a CLI runner, golden files, property tests, Playwright for a web UI, …) is chosen
    # per the project's charter/stack/adapter — `playwright` is NOT a first-class named type.
...
GROUNDING_OUT = _obj({
    ...
    "capability": _STR,                    # the target capability/module this change touches
...
# (e.g. "rewrite logic INSIDE the wrong module") is caught at the gate, not after the build.
CODE_PLAN_OUT = _obj({
...
VALIDATE_OUT = _obj({
    ...
    # AFTER-EVIDENCE is part of the deliverable, not optional: the captured proof the fix works,
    # POSTED to the GitHub issue (a UI screenshot via attach_image_to_issue, or text — stdout /
    # API response / test transcript — via post_comment). `evidence` lists what was posted (catbox
    # URL(s) and/or a one-line description of each comment); empty is a FAIL, never a pass.
    "evidence": _arr(_STR),
}, ["passed", "evidence"])
...
# The roster
# --------------------------------------------------------------------------- #
# Each agent carries a `tier` label (`fast` / `smart` / `deep`) — advisory metadata
# marking heavyweight-reasoning agents vs small discrete ones. Under the canonical
# Claude Code engine the whole loop runs in ONE session (that session's model IS the
# model), so the tier is descriptive, not a per-agent model selector.
ROSTER: dict[str, AgentSpec] = {
    ...
    "interop": AgentSpec(
        ...
        INTEROP_OUT, prompt_file="interop.md", tier="deep", charter_keys=["contracts"]),
    "security": AgentSpec(
    ...
    "architecture": AgentSpec(
        ...
        REVIEW_OUT, prompt_file="architecture.md", tier="deep",
        charter_keys=["is", "surfaces", "principles", "contracts", "stack"]),
    "ux": AgentSpec(
        "ux", "Review interface ergonomics + cross-surface consistency (UI pixels OR CLI/API/library shape).",
        REVIEW_OUT, prompt_file="ux.md", tier="deep", charter_keys=["project-kind", "surfaces"]),
    ...
    "design": AgentSpec(
        ...
        charter_keys=["scope", "surfaces", "principles", "stack"],
    ...
    "test-author": AgentSpec(
        ...
        charter_keys=["project-kind", "surfaces", "stack"],   # learns whether tests drive a UI, CLI, API, or library
