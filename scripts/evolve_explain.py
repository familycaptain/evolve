#!/usr/bin/env python3
"""Read-only Evolve lookup — fetch a run's gate packet + activity from the dashboard so the
...
decides, resolves, or mutates a gate.
    ...
    python3 scripts/evolve_explain.py 13               # loose-resolve -> ev-13, print its digest
    python3 scripts/evolve_explain.py ev-12f8541f      # exact instance id
    python3 scripts/evolve_explain.py ev-1 --events    # also show recent activity
    python3 scripts/evolve_explain.py ev-1 --json      # raw packet JSON (everything)

The GET endpoints need no auth; EVOLVE_SERVICE_TOKEN (from .env) is sent as Bearer if present —
never printed. The platform (dashboard host) base URL comes from $EVOLVE_SERVER_URL (or $EVOLVE_PLATFORM_URL); set
...
TOK = os.getenv("EVOLVE_SERVICE_TOKEN")
...
def main():
    ...
    ap.add_argument("item", nargs="?", help="run id or loose token (e.g. 13, ev-13, ev-12f8541f); omit/`list` to list")
