...
AUTH — this is the whole security model the operator asked for:
  ...
  * The dashboard base URL comes from $EVOLVE_SERVER_URL / $EVOLVE_PLATFORM_URL; nothing is hardcoded.
...
def main():
    ...
    try:
        ...
        if e.code == 403:
            sys.exit(f"403 — EVOLVE_DECIDE_TOKEN lacks admin role (or is wrong). {detail}")
