#!/usr/bin/env python3
"""Live-validation lifecycle controller (runs ON the test host).

Deploys a branch onto the test host's live dockerized Skipper, waits until it's actually serving,
and resets back to baseline. The deploy primitive is the SAME command the operator runs by hand —
...
REPO = os.path.expanduser(os.environ.get("EVOLVE_TARGET_REPO_PATH", "~/repos/<project>"))
BASELINE = "release"
