#!/usr/bin/env python3
"""LIVE acceptance for issue #36 — no web SPA form can native-submit / reload.

Bound test for ev-36. Runs ON the test host against the live dockerized Skipper
...
PWFILE = os.path.expanduser("~/.qa_pw")
...
def _goto_create_user(page):
    ...
    # OpenAI step auto-checks on mount; Next enables once state==ok (key present on the test host).
    try:
...
def test_onboarding(browser):
    """Real Onboarding account-creation form on a FRESH (pre-onboarding) test host — the
    ...
    VALIDU, VALIDP = USER, (PW or "testpass")
...
def is_fresh():
    """True if the test host is pre-onboarding (the onboarding form will render)."""
    try:
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            if fresh:
                # On a fresh host the app renders Onboarding, NOT Login. Onboarding FIRST
                # (its final real create makes USER/PW the admin -> host onboarded), THEN
                ...
                results.append({"scenario": "onboarding_no_reload", "passed": False,
                                "detail": "CANNOT VALIDATE: the test host is already onboarded so the "
    ...
    out = {
        ...
        "fresh_box": fresh,
        "note": ("All 3 forms live-exercised when run on a FRESH test host (run "
                 ...
                 "no-duplicate-submit). On an already-onboarded host the onboarding form can't "
