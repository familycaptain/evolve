#!/usr/bin/env python3
"""Test-host fixture snapshot/restore (Phase 1) — reproducible acceptance starting state.

The test host's DB persists across `skipper update`, so test data drifts run-to-run. This captures a
known baseline ONCE (`snapshot`) and restores it before each acceptance run (`reset`), so scenarios
always start from an identical state. The test host is the disposable validator, so a full-DB reset
is fine.
