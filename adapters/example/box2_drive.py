#!/usr/bin/env python3
"""box2_drive.py — thin 'hands' for driving the test host's live Skipper from an interactive
agent session (the BRAIN lives in the agent, NOT here — there is no LLM in this file). stdlib only.
...
agent driver issues one `say` per turn over SSH and reasons/judges in its own context.
"""
...
def signup(user, pw, display=""):
    st, res = _req("POST", "/api/onboarding/create-user", {
        ...
        "password": pw, "timezone": "America/New_York",
    })
