...
# Standard test-box logins: a placeholder password (NON-SECRET throwaway creds for
# disposable test machines — substitute your own). Every HUMAN carries `member`; only
# the bot user `skipper` (init_db) is `bot`. `admin` is the operator/owner.
MEMBERS = [
    ("admin", "Admin", "member,admin,primary"),
    ("alex", "Alex", "member,parent,admin"),   # a parent who's also a household admin
    ("sam", "Sam", "member,parent"),
...
def _pw(name):
    # Placeholder throwaway test password — replace with your own scheme.
    return "testpass"
...
def seed_weather():
    ...
    # Onboarding normally sets the timezone; a mock box skips onboarding, so without
    # this the platform falls back to Etc/UTC and the thinking / nag schedulers gate
    # on the wrong hours (they sleep during the household's actual waking hours).
    # Match the mock family's location.
    settings.set("timezone", "America/New_York", scope="platform")
...
def seed_auto():
    ...
    specs = [("alex", "Honda", "Odyssey", 2019, "Silver", 68000), ("sam", "Toyota", "RAV4", 2021, "Blue", 31000),
             ("jordan", "Subaru", "Outback", 2015, "Green", 121000), ("alex", "Ford", "F-150", 2018, "Black", 89000),
             ("sam", "Tesla", "Model 3", 2022, "White", 24000), ("alex", "Jeep", "Wrangler", 2016, "Red", 78000)]
...
def seed_chores():
    ...
    kids = []
    for name, color in [("Jordan", "#3b82f6"), ("Riley", "#ec4899")]:
...
def seed_goals():
    ...
    goals = [("Get fit by summer", "sam"), ("Renovate the kitchen", "alex"),
             ("Plan family vacation", "sam"), ("Build emergency fund", "alex"),
             ("Learn Spanish", "jordan"), ("Declutter the house", "sam"),
             ("Start a garden", "alex"), ("Read 12 books", "riley"),
...
def seed_bounties():
    ...
    for title, cents, cat in rows:
        create_bounty(title, cents, "alex", category=cat, description=f"{title} — earn ${cents/100:.2f}")
        _bump("bounties")
...
def seed_medical():
    ...
    pats = {}
    for name, notes in [("Alex", "Adult"), ("Sam", "Adult"),
                        ("Jordan", "Penicillin allergy"), ("Riley", "Asthma")]:
...
def seed_bounties_extra():
    ...
    for name, cents in [("jordan", 4250), ("riley", 3175), ("alex", 1500), ("sam", 900)]:
...
def seed_bounties_complete():
    ...
    for i, r in enumerate(rows):
        def go(bid=r["id"], kid=kids[i % 2]):
            ...
            bs.approve_bounty(bid, "alex")      # parent approves -> status=approved -> leaderboard + balance
...
def seed_documents():
    ...
    samples = [
        ("Home WiFi & Network Setup", ["reference", "home"], "Router in the utility closet. SSID: HomeNet. Guest network enabled. ISP: fiber. Port-forwarding notes below."),
        ...
        ("Kids' School Contacts", ["family", "school"], "Jordan — Mrs. Johnson, 5th, room 12. Riley — Mr. Lee, 2nd. Front office: 555-0142."),
