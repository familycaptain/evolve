...
Usage (async):
    ui = await UI.launch()
    await ui.login("alex", "testpass")
    await ui.open_settings(); await ui.settings_section("System")
    await ui.set_field("Location", "Riverton, US")
    await ui.save()
...
# Target host via SKIPPER_UI_BASE. The brain host's automated validation (Gate-2 AND Gate-3
# pre-verify) drives the test host; the operator's own manual testing is on a dedicated UAT host.
