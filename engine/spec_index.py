...
from engine.workspace import specs_root_for
...
# (brain host only), NOT the base requirements.
# --------------------------------------------------------------------------- #
import json
import logging
import math
...
# Cache of spec vectors on the brain host (keyed by record_id + content_hash; model-pinned). JSON,
# alongside the loop's $EVOLVE_STATE_DIR state. Env override for tests/other hosts.
_DEFAULT_CACHE = os.path.expanduser(os.path.join(os.getenv("EVOLVE_STATE_DIR") or "~/.evolve/runs", "spec_index_cache.json"))
...
if __name__ == "__main__":
    ...
    if args and args[0] == "--search":
        q = " ".join(args[1:]).strip()
        if not q:
            print('usage: python3 -m engine.spec_index --search "<text>"')
            raise SystemExit(2)
        ...
        if not cap:
            print("usage:\n  python3 -m engine.spec_index <capability>        # a capability's tree\n"
                  '  python3 -m engine.spec_index --search "<text>"     # cross-corpus nearest specs')
            raise SystemExit(2)
