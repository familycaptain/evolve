...
stdlib-only (urllib) so it runs in the minimal box-1 venv. Config via env:
    ...
    GITHUB_REPO    — owner/repo (from $GITHUB_REPO)
"""
import base64
import json
import os
...
def _load_dotenv_once() -> None:
    ...
    # repo root = the parent of this engine/ package (engine/github_connector.py -> engine -> root).
    # (Two dirnames — matches dashboard/server.py and scripts/evolve_adapter.py. A third overshoots
    # to the repo's parent dir, which silently misses .env in the standalone layout.)
...
def _token(repo: str | None = None) -> str:
    """The GitHub token for `repo`. A repo in a DIFFERENT account names its own token var via the
    registry's `token_env`; everything else uses GITHUB_TOKEN (GH_TOKEN as a legacy alias)."""
    _load_dotenv_once()
    if repo:
        try:
            from engine import repos as _repos
            var = _repos.repo_token_env(repo)
            tok = (os.getenv(var) or "").strip()
            if tok:
                return tok
        except Exception:
            pass
...
def _repo(repo: str | None = None) -> str:
    if repo or os.getenv("GITHUB_REPO"):
        return repo or os.getenv("GITHUB_REPO")
    try:                                   # fall back to the registry's primary repo
        from engine import repos as _repos
        return _repos.primary_repo() or DEFAULT_REPO
    except Exception:
        return DEFAULT_REPO
...
def _catbox_upload(image_path: str) -> str:
    ...
    with open(image_path, "rb") as fh:
        data = fh.read()
    boundary = "----evolveboundary" + os.urandom(12).hex()
    body = b"".join([
...
def _upload_image(image_path: str) -> str:
    """Upload an image and return its public URL. PLUGGABLE: set EVOLVE_IMAGE_UPLOAD_CMD to your own
    uploader (it's run with the image path as the final arg; the URL is read from its stdout). If unset,
    the DEFAULT is catbox.moe — a free, public, anonymous host."""
    cmd = (os.getenv("EVOLVE_IMAGE_UPLOAD_CMD") or "").strip()
    if cmd:
        import shlex, subprocess
        r = subprocess.run(shlex.split(cmd) + [image_path], capture_output=True, text=True, timeout=60)
        url = (r.stdout.strip().splitlines() or [""])[-1].strip()
        if not url.startswith("http"):
            raise RuntimeError(f"EVOLVE_IMAGE_UPLOAD_CMD returned no URL: {r.stdout[:160]} {r.stderr[:160]}")
        return url
    return _catbox_upload(image_path)


def attach_image_to_issue(number: int, image_path: str, caption: str = "",
                          repo: str | None = None, token: str | None = None) -> dict:
    """Upload an evidence screenshot and post it as an INLINE image comment on the issue. The image host
    is pluggable via `EVOLVE_IMAGE_UPLOAD_CMD`; the DEFAULT is **catbox.moe**, an external/public/anonymous
    host — the shot leaves your network and gets a permanent public URL (which is *why* GitHub renders it
    inline even on a private repo), so the default is only safe for NON-SENSITIVE / mock data. Point
    `EVOLVE_IMAGE_UPLOAD_CMD` at your own host to keep evidence private. Returns the created comment."""
    url = _upload_image(image_path)
    name = os.path.basename(image_path)
