...
flowchart TD
  ...
  %% reading code alone misattributes a UI symptom to the wrong code (a surface symptom can be
  %% produced by more than one code path). See what the USER sees first, then ground in the code behind THAT.
...
- **The funnel — cheap gates BEFORE the expensive spec phase (so it scales).** After
  ...
  **product charter + the target Capability's scope**) — *features only*, since a bug fixes
  already-accepted behavior. Then **Prioritize**
...
- **Gate-1 now OPENS with a security screen + empirical reproduction — BEFORE any code is read.**
  Reading code alone misattributes a UI symptom to the wrong code (the same on-screen element can be
  produced by more than one code path — e.g. a foreground flow vs. a background path — so a code-read
  can land on the wrong one and wrongly conclude "no issue"). So for a surfaced item, the spec phase
  opens with two new agents:
  1. **Security screen** (`security-screen`) — a subagent that reads **only the raw issue** and
     ...
     Gate 1.
  2. **Reproduce on evolve-test** (`reproduce`) — deploy the current staging branch (pre-fix) and
     recreate the **reported** symptom on its exact user-facing surface, **screenshot what the user
     sees**, and post it to the GitHub issue. *Could-not-reproduce is a first-class outcome* — a
     "couldn't reproduce" Gate-1 finding (already fixed? steps unclear? environment-specific?), never
     an invented fix. A **reproduced** item carries its proven `surface` into Grounding, which then
...
- **Gate 3 — verify (acceptance): merge is NOT done.** Auto-merge ships a *candidate* to the staging
  ...
  runs mock data and is separate from any production deployment, so verifying never disturbs a
  live instance.)
- **The build half fails closed.** A change reaches a *green* Gate 2 only when the
  implement agent actually changed code **and** included a runnable **bound test**, and
  evolve-test ran *that* test (not the engine's own suite) and it passed. A failed/empty/untested
...
- **The per-change process ends at the staging branch, not the world branch.** Promotion
  `$EVOLVE_STAGING_BRANCH → $EVOLVE_WORLD_BRANCH` (publish to the world) is a separate, operator-owned
  **release gate** — batched over many completed instances and verified on evolve-uat — so it lives outside
  this per-change graph.
- **Where the branches live (Option A — build on evolve-brain):**
  - The **implement agent builds on evolve-brain**, in an isolated worktree on a `feature/*`
...
```mermaid
flowchart LR
  merged(["Merged to the staging branch"]):::event --> verify[/"GATE 3 — verify (operator + PM test live on evolve-uat)"/]:::gate
...
- **Where it runs:** live today in the subscription `/loop` engine (`.claude/skills/evolve`, phase
  ...
  `github_connector.close_issue` does the close-on-verify). The production SDK graph adopts it as a
  follow-up — it would need a verify-packet builder, the `closeit` system handler, and the
  works/broken/abandon → resume wiring before the node can go live there.
