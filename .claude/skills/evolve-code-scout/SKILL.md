...
Play the **Code Scout** agent. Canonical instructions: read `agents/prompts/code-scout.md`.
...
Produce `CODE_PLAN_OUT` (`agents/registry.py`):
...
- `placement_notes[]` — **where shared logic should live (platform vs app)**; call out anything that
  ...
  is the highest-value output — it's what catches "rewrite a shared lookup inside one app".
