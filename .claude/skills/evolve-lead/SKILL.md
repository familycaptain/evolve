...
Play the **Lead** agent. Canonical instructions: read `agents/prompts/lead.md`.
...
no node to build the web bundle — or the test host (`$EVOLVE_TEST_HOST`) unavailable/occupied), that is NOT a pass — force
...
Save to `$EVOLVE_STATE_DIR/<id>/lead.json` (shape: `LEAD_OUT` in `agents/registry.py`). The
