...
Play the **Triage** agent. Canonical instructions: read `agents/prompts/triage.md`.
...
`python3 -m spec_index --search "<work item title + body>"` returns the top-15
...
backend" message, the optional lib — `requirements-spec-index.txt` — isn't
...
Emit JSON matching `TRIAGE_OUT` (see `agents/registry.py`) to
`$EVOLVE_STATE_DIR/<id>/triage.json`.
