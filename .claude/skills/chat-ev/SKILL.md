...
description: >
  ...
  (e.g. /chat-ev 39), fetch its live status + gate packet from the Evolve dashboard and run an
  interactive back-and-forth: explain what's proposed, answer questions, surface gaps/risks, and
  collaboratively REVISE the requirements BEFORE the operator decides the gate in the Evolve UI. This is
  the missing "assistant agent" step between an item reaching gate-1 and the operator approving it to build.
---
...
## 1. Fetch the item (live, from the Evolve dashboard)
...
- The dashboard URL + token come from the **environment** (`EVOLVE_SERVER_URL` / `EVOLVE_PLATFORM_TOKEN`,
  set in `.env`). Never hardcode or print them — this skill ships in a public repo.
- If the dashboard is unreachable (e.g. mid-deploy), run the same command on the brain host
  (`ssh $EVOLVE_BRAIN_HOST 'cd $EVOLVE_TARGET_REPO_PATH && python3 scripts/evolve_explain.py <id>'`) — it
  holds the same packet locally under `$EVOLVE_STATE_DIR/<n>/`. Say which source you used.

Read the digest yourself before replying. For the literal spec text or proposed diff, pull `--json`;
for full per-agent reasoning, read `$EVOLVE_STATE_DIR/<n>/{grounding,design,gate1,lead,review-*}.json` on
the brain host.

## 2. Open the conversation — orient, don't dump
Lead with a tight, plain-language brief: what this item IS (in terms of the target product and its
users), what the agents propose to build, and the **one or two things actually worth deciding** —
...
- Answer grounded in the ACTUAL code + the project's charter principles (`CHARTER.md`) — read files when
  it matters, and trust live code over the packet (say what you verified).
- Probe the ambiguities the spec left open ("per-user or shared?", "what happens on conflict?", "what's
  explicitly out of scope?"). Offer concrete options with a recommendation.
- Pressure-test against the project's charter values (`CHARTER.md`) — flag clearly if the proposal
  violates one.
...
goes on to test-host validation then Gate-3 verify).
...
- **The autonomous Evolve agents/loop CANNOT decide gates** — that's the whole point. The parent
  `EVOLVE_DECIDE_TOKEN` lives ONLY on evolve-admin (the operator's assistant machine, this one), never
  on the brain host where the loop runs; the loop holds only the service token, which the decision
  endpoint rejects. (Marking a gate `resolve`d *after* a build is still the engine's job, not part of
  deciding.)
- **Never embed the dashboard URL or any credential** in this skill, in commands you suggest, or in
  output — always via `$EVOLVE_SERVER_URL` / `.env`. Keep it clean for public distribution.
