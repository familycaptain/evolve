...
Play the **Spec-audit** agent. Canonical instructions: read `agents/prompts/spec-audit.md`.
You were spawned as a fresh subagent so your critique is INDEPENDENT of the author.

Read the spec at `$EVOLVE_STATE_DIR/<id>/spec*.json` (+ the design + grounding if useful). Hunt the
...
Emit JSON matching `SPEC_AUDIT_OUT` (`agents/registry.py`) — return it to the orchestrator.
