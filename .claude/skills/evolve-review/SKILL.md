...
your lens name into the path: `agents/prompts/security.md`, `…/architecture.md`,
...
Read the proposal + spec(s) at `$EVOLVE_STATE_DIR/<id>/`. Review ONLY through your lens; be specific and
skeptical. Emit concerns (`severity` + concrete `detail`) and an `approve` boolean — shape
`REVIEW_OUT` (interop uses `INTEROP_OUT`: a `conflicts` list). See `agents/registry.py`.
...
surfaces the charter declares + cross-surface consistency (and, for a headless/no-GUI project per the
charter's project-kind, interface ergonomics — the CLI flags / API shape / public signatures — rather
than pixels).
