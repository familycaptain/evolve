...
Play the **Design** agent. Canonical instructions: read `agents/prompts/design.md`.

Building on your **grounding**, reframe what's actually needed (not just the literal ask), set the
**approach**, and MAKE the key technical decisions (grounded in the real code) — honor the product's
...
You do this yourself (keep it in the conversation). Save to `$EVOLVE_STATE_DIR/<id>/design.json`
(shape: `DESIGN_OUT` in `agents/registry.py`).
