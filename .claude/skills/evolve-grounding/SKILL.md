...
Play the **Grounding** agent. Canonical instructions: read `agents/prompts/grounding.md`.
...
under the configured app dir (`$EVOLVE_APP_GLOB`); `platform` for the core) and run `python3 -m spec_index <cap>` —
...
`$EVOLVE_STATE_DIR/<id>/grounding.json` (shape: `GROUNDING_OUT` in `agents/registry.py`) so
